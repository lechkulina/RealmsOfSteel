...
ros::RawBufferPtr ros::ZIPArchiveEntry::decompress() {
    ...
    ROS_ERROR(boost::format("Unsupported compression method used for entry %s in archive %s") % getName() % file->getPath());
    return ros::RawBufferPtr();
}
...
bool ros::ZIPArchiveEntry::readFileHeader() {
    ...
    if (!file->seek(directoryHeader.headerOffset, FileOrigin_Begin) ||
        ...
        ROS_ERROR(boost::format("Failed to read file header signature for entry %s from archive %s") % getName() % file->getPath());
        return false;
    }
    if (fileHeader.signature != ZIPFileHeader::SIGNATURE) {
        ROS_ERROR(boost::format("Invalid file header signature set for entry %s in archive %s") % getName() % file->getPath());
        return false;
    }
    ...
    if (!file->readLittle(fileHeader.versionRequired) ||
        ...
        ROS_ERROR(boost::format("Failed to read file header for entry %s from archive %s") % getName() % file->getPath());
        return false;
    }
...
ros::RawBufferPtr ros::ZIPArchiveEntry::readUncompressedData() {
    ...
    if (!buffer || buffer->isNull()) {
        ROS_ERROR(boost::format("Failed to create destination buffer of size %d bytes for entry %s from archive %s")
    ...
    if (!file->seek(fileHeader.nameLength + fileHeader.extrasLength, FileOrigin_Current) ||
        ...
        ROS_ERROR(boost::format("Failed to read uncompressed data for entry %s from archive %s") % getName() % file->getPath());
        return RawBufferPtr();
    }
...
ros::RawBufferPtr ros::ZIPArchiveEntry::inflateCompressedData() {
    ...
    if (srcBuffer.isNull()) {
        ROS_ERROR(boost::format("Failed to create source buffer of size %d bytes for entry %s from archive %s")
                    % directoryHeader.compressedSize % getName() % file->getPath());
        return RawBufferPtr();
    }
    if (!file->seek(fileHeader.nameLength + fileHeader.extrasLength, FileOrigin_Current) ||
        !file->read(srcBuffer.at<void>(), srcBuffer.getSize())) {
        ROS_ERROR(boost::format("Failed to read compressed data for entry %s from archive %s") % getName() % file->getPath());
        return RawBufferPtr();
    }
    ...
    if (inflateInit2(&decompressor, -MAX_WBITS) != Z_OK) {
        ROS_ERROR(boost::format("Failed to create deflate decompressor for entry %s from archive %s - ZLIB error occured %s")
    ...
    if (result != Z_OK && result != Z_STREAM_END) {
        ROS_ERROR(boost::format("Failed to decompress defalte data for entry %s from archive %s - ZLIB error occured %s")
                    % getName() % file->getPath() % decompressor.msg);
        return RawBufferPtr();
    }
